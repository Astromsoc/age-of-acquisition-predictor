...
class InfererAPI:
    ...
    WORD_TEXT_PATTERN       = re.compile(r"[^a-z/.' -]*")
    ...
    def infer_text(self, text: str):
        # tokenize
        tokens = re.sub(self.WORD_TEXT_PATTERN, "", text.lower()).split(' ')
        # record each token's output result
        tokens2ages = dict()
        for token in tokens:
            # remove trailing period
            token = re.match(self.TRAILING_PERIOD_REGEX, token).group(1)
...
def main(args):
    ...
    # build inferer
    inferer = InfererAPI(ckpt=cfgs.ckpt, device=device)


    while True:
        # input random text
        age_range_str = input("\n\nPlease input the age group, formatted as (low, high):\n")
        m = re.match(inferer.AGE_RANGE_REGEX, age_range_str)
        ages = (float(m.group(1)), float(m.group(3)))
        text = input("\n\nPlease input the text for age prediction:\n")
        # inferrence
        avg_age, specifics = inferer.infer_text(text)
        # display
